...
void setup()
{
  ...
  delay(500);
  setColor(0, 255, 0); // Gr�n
  delay(300);
  setColor(0, 0, 255); // Blau
  delay(600);
  setColor(255, 255, 255); // Wei�
  delay(400);
  setColor(255, 255, 0); // Gelb
  delay(600);
  setColor(0, 255, 255); // T�rkis
  delay(500);
}
...
void loop()
{
  ...
  // Eine der gefunden Karten ausw�hlen
  if ( ! mfrc522.PICC_ReadCardSerial())
  {
    return;
  }

  // Eigene UID bilden. Dazu nehmen wir die 4(7)Byte Hex UID aus Block 0 und quetschen sie in einen sch�nen String.
...
*/
/* 
// �berpr�fe auf unterst�tzten PICC Typ und noch mehr Debug Kram
  if ( piccType != MFRC522::PICC_TYPE_MIFARE_MINI && piccType != MFRC522::PICC_TYPE_MIFARE_1K&& piccType != MFRC522::PICC_TYPE_MIFARE_1K) {
    Serial.println("Diese Tech-Demo ben�tigt MIFARE Classic Karten.");
    // RGB LED f�r 3,5sek auf BLAU
   setColor(0,0,255); // blau
    delay(3500);
    return;
  }
 */
// Den UID String vergleichen und eine entsprechende Aktion ausf�hren
  if( uID=="5b19d5ec" || uID=="4664d835")  {
     Serial.println("Access granted for uID " + uID);
    // RGB LED f�r 3,5sek auf GR�N
    setColor(0, 255, 0); // gr�n
    delay(3500);  
}
else {
       Serial.println("I can't let you do that Dave (" + uID + ")");
      // RGB LED f�r 3,5sek auf ROT
      setColor(255, 0, 0); // rot
      delay(3500);
  }
}
